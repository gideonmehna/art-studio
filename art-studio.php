...
function create_artwork_post_from_forminator($form_id, $response, $form_fields = null)
{
            ...
            if (!empty($files)) {
                ...
                if (file_exists($latest_file)) {
                    ...
                    $file_array = array(
                        ...
                        'tmp_name' => $latest_file,
                        'type' => mime_content_type($latest_file),
                    ...
                    if (!is_wp_error($attachment_id)) {
                        ...
                        error_log("Art Studio: Failed to create attachment - " . $attachment_id->get_error_message());
                    }
                } else {
                    error_log("Art Studio: File not found at path: " . $latest_file);
                }
            } else {
                error_log("Art Studio: No files found in uploads folder: " . $uploads_folder);
            }
        } else {
            error_log("Art Studio: No form submission folders found in: " . $forminator_upload_base);
        }
    }
    ...
    //     $attachment_id = media_handle_upload('upload-1', $post_id);

    //     if (!is_wp_error($attachment_id)) {
    //         set_post_thumbnail($post_id, $attachment_id);
    //         error_log("Art Studio: Image uploaded with ID - {$attachment_id}");
    //     } else {
    //         error_log("Art Studio: Image upload failed - " . $attachment_id->get_error_message());
    //     }
    // }
